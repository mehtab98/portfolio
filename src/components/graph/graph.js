...
const BarTextContent = ({ skillsets }) => {
  return (
    <div className="bar-text-content">
      {skillsets.map(skill => (
        <div className="text">{skill.skillsName}</div>
      ))}
    </div>
  );
};
...
const BarTextData = ({ skillsets }) => {
  return (
    <div className="bar-text-data">
      {skillsets.map(skill => (
        <div className="text">{skill.skillPercent}%</div>
      ))}
    </div>
  );
};
...
class Graph extends React.Component {
  ...
  renderBars() {
    ...
    return skills.map(skills => {
      ...
      return <Bar percent={percent} key={skills.skillsName} />;
    });
  }
    ...
    return (
      <div className="graph-wrapper">
        <div className="graph">
          ...
          <div className="bar-lines-container">{this.renderBars()}</div>
